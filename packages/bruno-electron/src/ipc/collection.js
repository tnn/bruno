...
const registerRendererEventHandlers = (mainWindow, watcher) => {
  ...
  ipcMain.handle('renderer:resolve-openapi-external-examples', async (_event, { spec, specFilePath }) => {
    ...
    const { makeSpecExampleReader } = require('../utils/openapi-external-examples');
    const readFile = makeSpecExampleReader({ sourceUrl: specFilePath });
    return brunoConverters.resolveExternalExamples(spec, { readFile });
  });
